...
interface NavItem {
  to: string;
  label: string;
  section?: string; // undefined = always visible (dashboard)
...
export function AppLayout({ children }: Props) {
  const { user, workshop, permissions, logout } = useAuthStore();
  const navigate = useNavigate();

  const visibleNav = NAV.filter(({ section }) =>
    !section || hasPermission(user, permissions, section),
  );
      ...
      <aside className="w-60 flex-shrink-0 bg-slate-900 flex flex-col">
        ...
        <div className="flex items-center gap-3 px-5 h-16 border-b border-slate-700/60 flex-shrink-0">
          ...
          <div className="min-w-0">
            <p className="text-sm font-bold text-white tracking-tight truncate">GarageSaaS</p>
            {workshop && (
              <p className="text-xs text-slate-400 truncate leading-tight">{workshop.name}</p>
            )}
          </div>
        </div>
        ...
        {/* User footer */}
        <div className="border-t border-slate-700/60 px-4 py-4 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              title="Sign out"
              className="p-1.5 text-slate-500 hover:text-slate-200 rounded-lg hover:bg-slate-800 transition-colors flex-shrink-0"
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round"
