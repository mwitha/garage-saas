...
export function CustomerForm({ open, onClose, title, defaultValues, onSubmit, isPending, error }: Props) {
  const { register, handleSubmit, reset, formState: { errors } } = useForm<CustomerFormData>({
    resolver: zodResolver(schema),
    defaultValues,
  });

  // Sync defaultValues when the modal opens (edit mode)
  useEffect(() => {
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate>
          <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
            ...
            <Field label="Address" error={errors.address?.message}>
              <Input {...register('address')} hasError={!!errors.address} placeholder="123 Galle Road" />
            </Field>
...
// Helper: convert a Customer record to CustomerFormData for edit pre-fill
export function customerToFormData(c: Pick<import('../../types').Customer,
  'name' | 'phone' | 'email' | 'city' | 'address' | 'nic_number' | 'notes'>
): CustomerFormData {
  return {
    name:       c.name,
    phone:      c.phone,
